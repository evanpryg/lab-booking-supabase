...
export async function newBooking(el) {
  ...
  el.innerHTML = U.card(`
    <form id="bk" class="p-6 space-y-4">
      <div>
        <label class="text-xs font-medium text-slate-500">Laboratorium</label>
        <select name="lab_id" required class="mt-1 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:ring-4 focus:ring-brand-500/15 focus:border-brand-500 outline-none">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div><label class="text-xs font-medium text-slate-500">Tanggal</label>
          <input name="tanggal" type="date" required min="${U.todayISO()}" class="mt-1 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:ring-4 focus:ring-brand-500/15 focus:border-brand-500 outline-none"></div>
        <div><label class="text-xs font-medium text-slate-500">Jam Mulai</label>
          <input name="jam_mulai" type="time" required class="mt-1 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:ring-4 focus:ring-brand-500/15 focus:border-brand-500 outline-none"></div>
        <div><label class="text-xs font-medium text-slate-500">Jam Selesai</label>
          <input name="jam_selesai" type="time" required class="mt-1 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:ring-4 focus:ring-brand-500/15 focus:border-brand-500 outline-none"></div>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div><label class="text-xs font-medium text-slate-500">Jumlah Peserta</label>
          <input name="jumlah_peserta" type="number" min="1" max="30" required class="mt-1 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:ring-4 focus:ring-brand-500/15 focus:border-brand-500 outline-none" placeholder="mis. 30"></div>
        <div><label class="text-xs font-medium text-slate-500">Kelas</label>
          <input name="kelas" type="text" class="mt-1 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:ring-4 focus:ring-brand-500/15 focus:border-brand-500 outline-none" placeholder="mis. XI IPA 1"></div>
      </div>
      <div><label class="text-xs font-medium text-slate-500">Keperluan</label>
        <textarea name="keperluan" rows="2" class="mt-1 w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm focus:ring-4 focus:ring-brand-500/15 focus:border-brand-500 outline-none" placeholder="mis. Praktikum jaringan komputer"></textarea></div>
      <div id="equip-wrap" class="hidden">
        <label class="text-xs font-medium text-slate-500">Alat yang dipinjam (opsional)</label>
  ...
  form.lab_id.addEventListener('change', async () => {
    ...
    equipEl.innerHTML = eq.map((e) => `
      <label class="flex items-center gap-2 text-sm border border-slate-200 rounded-xl px-3 py-2 cursor-pointer hover:bg-brand-50">
        <input type="checkbox" value="${e.id}" class="rounded text-brand-600"> ${U.escapeHtml(e.nama)}</label>`).join('');
    ...
    <div class="bg-white rounded-2xl border border-slate-200/70 shadow-card p-4 flex items-start gap-4">
      ...
      <div class="flex-1 min-w-0">
        ...
        ${allowCancel && ['menunggu', 'disetujui'].includes(b.status)
          ? `<button data-cancel="${b.id}" class="mt-2 text-xs font-medium text-rose-600 hover:underline">Batalkan booking</button>` : ''}
      </div>
    </div>`).join('')}</div>`;
